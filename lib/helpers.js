...
let _ = {
  ...
  '_reset': function () { _defaultEnvID = null; },
  ...
  'findMagicConfigFileID': function (pathToMatch) {
    if (_defaultEnvID) { return _defaultEnvID; }
    if (!pathToMatch) { pathToMatch = _.rootPath; }
    ...
    fileList.some((currentFile) => {
      if (
        path.extname(currentFile) !== '.json' &&
        path.extname(currentFile) !== '.js'
      ) { return false; }

      try {
        envConfig = _.loadRef(_.getConfigFilePath(currentFile));
      } catch (err) {
      ...
      if (envConfig['_path'] === pathToMatch) {
        envID = path.basename(currentFile, path.extname(currentFile));
        return true;
      }
      return false;
    });
  ...
  'loadRef': function (filename) {
    let ext = path.extname(filename);
    if (ext === '') {
      return _.loadRef(filename +'js') || _.loadRef(filename +'.json');
    }
    if (!fs.existsSync(filename)) { return false; }
    if (ext === '.js') { return require(filename); }
