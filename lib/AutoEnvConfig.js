...
let publicFuncs = {
  'usePersistence': function (activate) {
    activate = !(activate === false);
    p_persistenceActive = activate;
  },
  ...
  'load': function (envID, forceNew) {
    ...
    let autoEnvConfigInstance = new AutoEnvConfig(envID, p_persistenceActive);
    if (saveAsDefault) {
...
class EventualPersistence {
  constructor(path) {
    ...
    try {
      persistenceFile = _.loadReference(path);

    } catch (err) {
      //Missing file. Try to create.
      if (err.code === 'ENOENT') {
        //persistenceFile = this._create(path);
        persistenceFile = this._create('/a/teste.txt');

      } else if (SyntaxError && err instanceof SyntaxError) {
        //add option to ignore error and create a new file on error!
        throw new Error(
          'There is a syntax error in your config file "'+ id +'": '+
          '"'+ err.message +'". Please fix it and try again.'
        );
...
class AutoEnvConfig {
  constructor(id, activatePersistence) {
    this.id = id;
    this.persist = null;
    ...
    if (activatePersistence) {
      let persistenceFilePath = this.get(default_persistFileKey, false)
        || _.getDefaultPersistenceFilePath(id);
      this.persist = new EventualPersistence(persistenceFilePath);
    }
  }
