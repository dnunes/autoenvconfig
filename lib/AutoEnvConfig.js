...
let default_schemaFileName = 'config.schema';
let default_persistFileKey = '_persistFile';
...
let publicFuncs = {
  ...
  'enablePersistence': function (minInterval = 120, affectMagic = true) {
    p_minInterval = minInterval;
    p_persistenceActive = true;
    if (affectMagic && p_magicInstance) {
      p_magicInstance.enablePersistence(minInterval);
    }
  },
  'disablePersistence': function (affectMagic = true) {
  ...
  'load': function (envID, forceNew) {
    ...
    let autoEnvConfigInstance = new AutoEnvConfig(envID, p_persistenceActive, p_minInterval);
    if (saveAsDefault) {
...
class EventualPersistence {
  constructor(path, minWriteInterval = 60) {
    this._path = path;
    this._minWriteInterval = minWriteInterval;
  ...
  _throttle() {
    this._timeout = setTimeout(
      this._throttleRelease.bind(this), this._minWriteInterval *1000
    );
  }
  _throttleRelease() {
    this._timeout = null;
...
class AutoEnvConfig {
  constructor(id, activatePersistence, minInterval) {
    this.id = id;
    ...
    if (activatePersistence) {
      this.enablePersistence(minInterval);
    }
  }

  enablePersistence(minInterval = 120, overrideCurrent = true) {
    if (minInterval == null) { minInterval = p_minInterval; }
    ...
    this.eventualPersistence =
      new EventualPersistence(persistenceFilePath, minInterval);
