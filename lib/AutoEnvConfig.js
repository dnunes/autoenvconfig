...
let _schema = {
  ...
  'get': function () {
    ...
    let schema;
    try {
      schema = _.loadRef(schemaPath) || _.loadRef(schemaPath +'.js');
    } catch (err) {
    ...
    if (!schema) {
      throw new Error(`There is no "${schemaFileName}" file in your envs folder!`);
    }
    return schema;
  },
...
class AutoEnvConfig {
  constructor(id, activatePersistence, minPersistInterval) {
    ...
    if (envFile === false) {
      throw new Error(`The specified config file "${id}" does not exist`);
    }
  ...
  enablePersistence(minPersistInterval, overrideMemory = true) {
  ...
  disablePersistence() {
    if (this.eventualPersistence) { this.eventualPersistence._kill(); }
    this.eventualPersistence = null;
  }
